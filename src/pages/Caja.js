...
const generarPDF = (info) => {
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.text(`Fecha de expedición: ${fecha}`, pageWidth - 20, y, {
    align: "right",
  });

  y += 18;
  doc.setFontSize(14);
  doc.text("RECIBO DE PAGO POR ANUALIDAD", pageWidth / 2, y, {
    align: "center",
  });
  ...
  /* Monto destacado */
  y += 20;
  doc.rect(25, y - 10, pageWidth - 50, 22);
  ...
  //doc.save(`Recibo_${info.usuario}_${info.numContrato}_${info.anioPago}.pdf`);
};
...
export default function CajaHome() {
    ...
    fetch(API_SELECT, {
      ...
      .then((data) => {
        const normalized = data.map((c) => ({
          ...c,
          id_contrato: c.id_contrato || c.id_Contrato,
        }));
    ...
    fetch(API_SELECT, {
      ...
      .then((data) => {
        const normalized = data.map((a) => ({
          ...a,
          id_contrato: a.id_contrato || a.id_Contrato,
          id_usuario: a.id_usuario || a.id_Usuario,
        }));
  ...
  /* ADEUDOS POR CONTRATO */
  const getAdeudosPorContrato = (id_usuario) => {
    const contratosUser = contratos.filter(
      (c) => String(c.id_usuario) === String(id_usuario)
    );

    return contratosUser.map((c) => {
      const adeudosContrato = adeudos
        .filter(
          (a) =>
            String(a.id_usuario) === String(id_usuario) &&
            String(a.id_contrato) === String(c.id_contrato) &&
            a.estado === "pendiente"
        )
        .map((a) => ({
          anio: Number(a.anio),
      ...
      return {
        ...
        contrato: c.num_contrato,
        adeudos: adeudosContrato,
      };
    });
  };

  /* MODAL DE PAGO */
  const abrirModalPago = (usuario, contrato, adeudo) => {
    Swal.fire({
      ...
      html: `
        <div style="text-align:left; font-size:16px;">
          ...
          <div style="padding:15px; border:1px solid #ccc; border-radius:8px; background:#f8f9fa;">
            <h3 style="margin:0; color:#0f4c75;">Datos del Usuario</h3>
            <p><b>Usuario:</b> ${usuario.Nombre} ${usuario.Apellido_pat} ${usuario.Apellido_mat}</p>
            <p><b>Contrato:</b> ${contrato.contrato}</p>
            <p><b>Año a pagar:</b> ${adeudo.anio}</p>
          </div>

          <div style="margin-top:20px; display:flex; flex-direction:column; gap:15px;">
            <div>
              <label><b>Monto:</b></label>
              <input id="monto_pago" type="number" class="swal2-input" style="width:100%;" />
            </div>

            <div>
              <label><b>Método de pago:</b></label>
              <select id="metodo_pago" class="swal2-input" style="width:100%; height:45px;">
            ...
            <div>
              ...
              <textarea id="observaciones" class="swal2-textarea" style="height:80px;"></textarea>
            </div>
          </div>
      ...
      preConfirm: () => {
        ...
        if (!monto || !metodo) {
          Swal.showValidationMessage("Completa monto y método de pago");
          return false;
        }

        return { monto, metodo, obs };
      },
    }).then(async (result) => {
      if (!result.isConfirmed) return;

      const { monto, metodo, obs } = result.value;
      ...
      if (!resPago.ok || dataPago.error) {
        return Swal.fire("Error", "No se pudo registrar el pago", "error");
      }
      ...
      generarPDF({
        usuario: `${usuario.Nombre} ${usuario.Apellido_pat} ${usuario.Apellido_mat}`,
        numContrato: contrato.contrato,
        anioPago: adeudo.anio,
        montoPago: monto,
        metodoPago: metodo,
        observaciones: obs,
      });
  ...
  return (
    <div style={{ width: "90%", margin: "40px auto", textAlign: "center" }}>
      <h1>Bienvenido, Caja</h1>
      <p>Consulta rápida de usuarios con adeudos</p>
      ...
      {busqueda.length > 0 && (
        ...
        >
          ...
          <tbody>
            {resultados.map((u) => {
              const contratosUser = getAdeudosPorContrato(u.id_usuario);
              const abierto = openRows[u.id_usuario];
              ...
              return (
                <React.Fragment key={u.id_usuario}>
                  <tr style={{ borderBottom: "1px solid #eee" }}>
                    ...
                    <td>
                      <button
                        onClick={() => toggleRow(u.id_usuario)}
                        style={{
                          padding: "6px 12px",
                          background: "#1b7bd1",
                          color: "white",
                  ...
                  {abierto && (
                    <tr>
                      <td colSpan="5" className="anim-expand">
                        {contratosUser.map((c) => (
                          <div
                            key={c.id_contrato}
                            style={{
