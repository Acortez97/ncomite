...
export default function RegistroAportacionVoluntaria() {
  ...
  const guardarAportacion = async () => {
    ...
    Swal.fire("Éxito", "Aportación registrada correctamente", "success");
...
function numeroALetras(num) {
  ...
  const generarPDF = (info) => {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(14);
    doc.text("RECIBO DE APORTE VOLUNTARIO", pageWidth / 2, y, {
      align: "center",
    });
    ...
    doc.setFont("helvetica", "normal");
    doc.text(
      "El presente documento certifica el pago de Aportación Voluntaria del servicio de agua potable,",
      pageWidth / 2,
      y,
      { align: "center" }
    );
    ...
    /* ===================== GUARDAR ===================== */
    doc.save(`Recibo_AportacionV_${info.usuario}_${info.numContrato}.pdf`);
