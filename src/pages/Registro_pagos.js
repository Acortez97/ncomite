...
export default function RegistroPagos() {
    ...
    useEffect(() => {
        fetch(API_SELECT_USUARIOS, {
            ...
            body: JSON.stringify({
                select:
                    'id_usuario, CONCAT_WS(" ", Nombre, Apellido_pat, Apellido_mat) AS Contratante',
                table: "usuarios",
                orderBy: "Nombre ASC",
            }),
        })
    ...
    useEffect(() => {
        ...
        const contrato = contratos.find(
            (c) => c.id_contrato == contratoSeleccionado
        );

        if (!contrato?.Fecha_contrato) return;

        // Generar adeudos automáticos según fecha del contrato
    ...
    const generarPDF = (info) => {
        ...
        doc.setFontSize(17);
        doc.text("Comité del Agua Potable", pageWidth / 2, y, {
            align: "center",
        });
    ...
    return (
        <div style={containerStyle}>
            ...
            >
                ...
                {/* ========== SELECT DE AÑOS PENDIENTES DE ADEUDO ========== */}
                <select
                    required
                    value={anioPago}
